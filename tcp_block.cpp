...
void GetMyAddress(const char* dev){
	struct ifreq ifrq;
...
int MakeRstPacket(uint8_t* packet, uint32_t seq, uint32_t ack, int direction ){
...
int MakeFinPacket(uint8_t* packet, uint32_t seq, uint32_t ack){
	struct libnet_ethernet_hdr * ether_header=(struct libnet_ethernet_hdr *)packet;
	struct libnet_ipv4_hdr * ip_header = (struct libnet_ipv4_hdr *)(packet +sizeof(struct libnet_ethernet_hdr));
	struct libnet_tcp_hdr * tcp_header=(struct libnet_tcp_hdr *)(packet+sizeof(struct libnet_ethernet_hdr)+ip_header->ip_hl*4);
	...
	ip_header->ip_src.s_addr=tmp_ip;
	ip_header->ip_tos=0x44;
	ip_header->ip_len=htons(ip_header->ip_hl*4+tcp_header->th_off*4+MESSAGE_LEN);
	ip_header->ip_ttl=0xff;
	ip_header->ip_sum=0;
	...
	tcp_header->th_sum=0;
	tcp_header->th_urp=0;
	strncpy((char*)data_ptr, (char*)message, MESSAGE_LEN);
}

int BlockTcp(uint8_t* packet, uint32_t seq, uint32_t ack, uint32_t header_len, uint32_t data_len, pcap_t* handle){
	uint8_t fd_packet[MTU] = {0,}, bk_packet[MTU] = {0,};
	memcpy(fd_packet, packet, MTU);
	memcpy(bk_packet, packet, MTU);
	MakeRstPacket(fd_packet, htonl(ntohl(seq)+data_len), ack, FORWARD);
	MakeRstPacket(bk_packet, ack, htonl(ntohl(seq)+data_len), BACKWARD);
	pcap_inject(handle, fd_packet, header_len);
	pcap_inject(handle, bk_packet, header_len);
}

int BlockHttp(uint8_t* packet, uint32_t seq, uint32_t ack, uint32_t header_len, uint32_t data_len, pcap_t* handle){
	uint8_t fd_packet[MTU] = {0,}, bk_packet[MTU] = {0,};
	memcpy(fd_packet, packet, MTU);
	memcpy(bk_packet, packet, MTU);
	MakeRstPacket(fd_packet, htonl(ntohl(seq)+data_len), ack, FORWARD);
	MakeFinPacket(bk_packet, ack, htonl(ntohl(seq)+data_len));
	pcap_inject(handle, fd_packet, header_len);
	pcap_inject(handle, bk_packet, header_len);
}

int CheckPacket(uint8_t* packet, pcap_t* handle){
	int pkt_type = 0;
	...
	if(pkt_type == TCP_PACKET) BlockTcp(packet, seq, ack, header_len, data_len, handle);
    else if(pkt_type == HTTP_PACKET) BlockHttp(packet, seq, ack, header_len, data_len, handle);
...
int main(int argc, char * argv[]){
	...
	while (true) {
    	...
    	CheckPacket((uint8_t*)packet, handle);  	
  }
  return 0;
}
